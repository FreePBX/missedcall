	...
	foreach($dialarray as $ext => $dialsts){
		...
		if(!isset($dialsts['ANSWER']) && isset($dialsts['MISSED']) ){
			...
			if($mc_params['enable']){// check only enable extension
				...
				if ($dialsts['CallType'] == 'ringgroup' && $mc_params['ringgroup']){
					$send_notice = true;
				}
				if ($dialsts['CallType'] == 'queue' && $mc_params['queue']){
					$send_notice = true;
				}
				// Send email now 
				if($send_notice){
					$McObj->sendEmail($mc_params['email'],$ext,$dialsts['callerid'],$dialsts['calleridname']);
				}else { 
					dbug(" No Email sent ");
				}
